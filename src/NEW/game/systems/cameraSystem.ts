...
const cameraSystem = (ecs: Ecs, ctx: CanvasRenderingContext2D) => {
  ...
  for (const camId of cameras) {
    ...
    const targetTransform = ecs.getComponent<TransformComponent>(
      cam.target,
      'transform',
    );
    if (!targetTransform) {
    ...
    const targetPos = {
      x:
        ...
        ctx.canvas.width / (2 * cam.zoom) +
        cam.offset.x,
      y:
        targetTransform.position.y -
        ctx.canvas.height / (2 * cam.zoom) +
        cam.offset.y,
    };
    ...
    if (cam.bounds) {
      cam.position.x = Math.max(
        cam.bounds.minX,
        Math.min(cam.position.x, cam.bounds.maxX),
      );
      cam.position.y = Math.max(
        cam.bounds.minY,
        Math.min(cam.position.y, cam.bounds.maxY),
      );
    }
  }
};
